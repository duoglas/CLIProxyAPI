...
func assertRedisProtocolBannedOnPipe(t *testing.T, server *Server, remote net.Addr, args ...string) {
	t.Helper()
	...
	wrappedConn := &remoteAddrConn{Conn: serverConn, remoteAddr: remote}
	...
	if errWrite := writeTestRESPCommand(clientConn, args...); errWrite != nil {
		t.Fatalf("failed to write command on banned connection: %v", errWrite)
	}
	msg, err := readTestRESPError(reader)
	if err != nil {
		t.Fatalf("failed to read banned error: %v", err)
	}
	if !strings.HasPrefix(msg, "ERR IP banned due to too many failed attempts. Try again in") {
		t.Fatalf("unexpected banned error: %q", msg)
	}
}
...
func TestRedisProtocol_IPBan_MirrorsManagementPolicy(t *testing.T) {
	...
	assertRedisProtocolBannedOnPipe(t, server, fakeRemote, "LPOP", "queue")
}
...
func TestRedisProtocol_AUTH_IPBan_BlocksCorrectPasswordDuringBan(t *testing.T) {
	...
	assertRedisProtocolBannedOnPipe(t, server, fakeRemote, "AUTH", managementPassword)
}
...
func TestRedisProtocol_LOCALHOST_AUTH_IPBan_BlocksCorrectPasswordDuringBan(t *testing.T) {
	...
	bannedConn, errDialBanned := net.DialTimeout("tcp", addr, time.Second)
	if errDialBanned != nil {
		t.Fatalf("failed to dial redis listener after ban: %v", errDialBanned)
	}
	t.Cleanup(func() { _ = bannedConn.Close() })
	bannedReader := bufio.NewReader(bannedConn)
	_ = bannedConn.SetDeadline(time.Now().Add(5 * time.Second))

	if errWrite := writeTestRESPCommand(bannedConn, "AUTH", managementPassword); errWrite != nil {
		t.Fatalf("failed to write AUTH command with correct password after ban: %v", errWrite)
	}
	msg, err := readTestRESPError(bannedReader)
	if err != nil {
