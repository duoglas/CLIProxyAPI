...
const (
	...
	codexResponseBodyDrainGrace = 100 * time.Millisecond
)
...
func drainCodexResponseBodyAfterCompleted(body io.ReadCloser) error {
	if body == nil {
		return nil
	}
	...
	go func() {
		_, errCopy := io.Copy(io.Discard, body)
		done <- errCopy
	}()

	timer := time.NewTimer(codexResponseBodyDrainGrace)
	defer timer.Stop()
...
func (e *CodexExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	for scanner.Scan() {
		...
		if errDrain := drainCodexResponseBodyAfterCompleted(httpResp.Body); errDrain != nil {
			log.Errorf("codex executor: close completed response body error: %v", errDrain)
		}
		bodyClosed = true
		return resp, nil
	}
