...
func applyClaudeHeaders(r *http.Request, auth *cliproxyauth.Auth, apiKey string, stream bool, extraBetas []string, cfg *config.Config) {
	...
	// Connection header is technically hop-by-hop and ignored in HTTP/2, but real
	// Claude Code clients still set "keep-alive" — matching their fingerprint here
	// is more important than the protocol purity argument.
...
//
// NOTE: when `experimentalCCHSigning` is true (default for OAuth tokens in
// applyCloaking), cch is fixed to "00000" so that the injected billing header is
// byte-identical across every turn in the same conversation. This is required
// for Anthropic's extended-thinking signature to validate on replay — a drifting
// system prompt between turns produces "Invalid signature in thinking block" (400).
...
func applyCloaking(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth, payload []byte, model string, apiKey string) []byte {
	...
	// Additionally, skip system-prompt rewriting for continuation turns that replay a
	// signed assistant `thinking` block: Anthropic validates the signature against
	// the system+tools context seen when the thinking was generated, and even with
	// cch=00000 we don't want any drift on follow-up turns. The first turn (no
	// signed thinking yet) still gets the full cloaking treatment.
	if !strings.HasPrefix(model, "claude-3-5-haiku") && !hasSignedAssistantThinking(payload) {
		billingVersion := helps.DefaultClaudeVersion(cfg)
		if cfg != nil && billingVersion == "" {
