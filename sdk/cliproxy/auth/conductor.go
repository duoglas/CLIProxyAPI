...
func (m *Manager) ExecuteStream(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, error) {
	...
	return m.executeStreamWithRouteModel(ctx, normalized, req.Model, req, opts)
}

func (m *Manager) ExecuteStreamWithRouteModel(ctx context.Context, providers []string, routeModel string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, error) {
	normalized := m.normalizeProviders(providers)
	if len(normalized) == 0 {
		return nil, &Error{Code: "provider_not_found", Message: "no provider supplied"}
	}
	return m.executeStreamWithRouteModel(ctx, normalized, routeModel, req, opts)
}
...
func (m *Manager) executeStreamWithRouteModel(ctx context.Context, providers []string, routeModel string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, error) {
	...
	var lastErr error
	for attempt := 0; ; attempt++ {
		result, errStream := m.executeStreamMixedOnceForRoute(ctx, providers, routeModel, req, opts, maxRetryCredentials)
		if errStream == nil {
			return result, nil
		}
		lastErr = errStream
		wait, shouldRetry := m.shouldRetryAfterError(errStream, attempt, providers, routeModel, maxWait)
		if !shouldRetry {
			break
		}
		if errWait := waitForCooldown(ctx, wait); errWait != nil {
...
func (m *Manager) executeStreamMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (*cliproxyexecutor.StreamResult, error) {
	return m.executeStreamMixedOnceForRoute(ctx, providers, req.Model, req, opts, maxRetryCredentials)
}

func (m *Manager) executeStreamMixedOnceForRoute(ctx context.Context, providers []string, routeModel string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (*cliproxyexecutor.StreamResult, error) {
	if len(providers) == 0 {
		return nil, &Error{Code: "provider_not_found", Message: "no provider supplied"}
	}
	routeModel = strings.TrimSpace(routeModel)
	...
	for {
		...
		execModels := models
		execPooled := pooled
		if strings.TrimSpace(req.Model) != "" && !strings.EqualFold(strings.TrimSpace(routeModel), strings.TrimSpace(req.Model)) {
			execModels = []string{req.Model}
			execPooled = false
		}
		attempted[auth.ID] = struct{}{}
		streamResult, errStream := m.executeStreamWithModelPool(execCtx, executor, auth, provider, req, opts, routeModel, execModels, execPooled)
		if errStream != nil {
			if errCtx := execCtx.Err(); errCtx != nil {
				return nil, errCtx
			}
...
func (m *Manager) MarkResult(ctx context.Context, result Result) {
	...
	m.mu.Unlock()
	if m.scheduler != nil {
		if modelStateSnapshot != nil && m.scheduler.applyModelStateUpdate(result.AuthID, result.Provider, result.Model, modelStateSnapshot, authSnapshot) {
			// Fast path updated the affected model shard without rebuilding every scheduler shard.
		} else if authSnapshot != nil {
